...
#get backtrace path for matrix (returns some optimal alignment)
...
while not @infile.eof 
  string_buffer1 = @infile.gets().strip #TODO: FASTA needs more OOP?
  ...
  if string_buffer1.index('>').nil?  
    ...
    lines_counter += 1
    lines_array[lines_counter - 1] = "" #TODO: not very elegant    
  end
end
puts "got #{lines_counter} lines from input file" 
@infile.close #we don't need this handle to be opened any more
